 ...
 * @since Aug 21, 2009
 */
public class SPFilter_copy implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain)
        ...
        if (!postMethod && !logOutRequest) {
            ...
            //if (userPrincipal != null) {
            	...
            	if (blacklist == null || !blacklist.crontains(sb.toString())) {*/
            		/*if (isNotNull(url_param)) {
	            		request.getRequestDispatcher(url_param).forward(request, response);
	            	} else {
	            		filterChain.doFilter(servletRequest, servletResponse);
            ...
            //} else {
		        ...
		        try {
		        	if (isNotNull(url_param)) {
		        		...
		        		// TODO: use the handlers to generate the request
		        		AuthnRequestType authnRequest = createSAMLRequest(fullPath, this.serviceURL, this.identityURL);
		        		sendRequestToIDP(authnRequest, relayState, response);
		        	}
