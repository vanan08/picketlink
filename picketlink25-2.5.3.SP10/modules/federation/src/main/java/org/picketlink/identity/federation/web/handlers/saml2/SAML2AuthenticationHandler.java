...
import com.prudential.SPFilter;
...
public class SAML2AuthenticationHandler extends BaseSAML2Handler {
    ...
    private class SPAuthenticationHandler {
        ...
        public void handleStatusResponseType(SAML2HandlerRequest request, SAML2HandlerResponse response)
                throws ProcessingException {
        	logger.info("SAML2AuthenticationHandler - handleStatusResponseType()");
            HTTPContext httpContext = (HTTPContext) request.getContext();
            ...
            Principal userPrincipal = handleSAMLResponse(responseType, httpContext, response);
            if (userPrincipal == null) {
                ...
                logger.info(GeneralConstants.PRINCIPAL_ID + " : " + userPrincipal);
        ...
        private Principal handleSAMLResponse(ResponseType responseType, HTTPContext httpContext, SAML2HandlerResponse response)
                throws ProcessingException {
            ...
            for (StatementAbstractType statement : statements) {
                if (statement instanceof AttributeStatementType) {
                    ...
                    roles.addAll(getRoles(attributeStatement, httpContext));
                }
            }
         ...
         * @return
         */
        private List<String> getRoles(AttributeStatementType attributeStatement, HTTPContext httpContext) {
            ...
            for (ASTChoiceType obj : attList) {
                ...
                if (attr.getName().equals("timeout")) {
                	SPFilter.limitSeconds = Long.parseLong((String) attr.getAttributeValue().get(0));
                	continue;
                } else if (attr.getName().equals("blacklist")) {
                	List<String> blacklist = new ArrayList<String>();
                	HttpSession session = httpContext.getRequest().getSession(false);
                	if (session != null) {
                		String value = (String) attr.getAttributeValue().get(0);
                		if (value != null && value.length() > 0) {
                			blacklist.addAll(Arrays.asList(value.trim().split(",")));
                		}
